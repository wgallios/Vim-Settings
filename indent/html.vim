...
fun! <SID>HtmlIndentOpen(lnum, pattern)
    let s = substitute('x'.getline(a:lnum), '.\{-}\(\(<\)\('.a:pattern.'\)\>\)', "\1", 'g')
...
fun! <SID>HtmlIndentSum(lnum, style)
    ...
    if '' != &syntax && synIDattr(synID(a:lnum, 1, 1), 'name') =~ '\(css\|java\).*' && synIDattr(synID(a:lnum, strlen(getline(a:lnum)), 1), 'name') =~ '\(css\|java\).*'
        if a:style == match(getline(a:lnum), '^\s*}')
